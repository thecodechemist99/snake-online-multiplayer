...
import Game from "./class_game.mjs";
import Player from "./class_player.mjs";
import Fruit from "./class_fruit.mjs";
...
let fruit = new Fruit(
  Math.floor(Math.random() * grid.width) * grid.fieldSize,
  Math.floor(Math.random() * grid.height) * grid.fieldSize
);
